...
class PatientService {
  ...
  Future<List<UserModel>> getAllpatients() async {
    ...
    if (response.statusCode == 200) {
      for (var u in jsonData) {
        UserModel pat = UserModel.fromJson(jsonData);
        patients.add(pat);
      }
    } else {
  ...
  Future<UserModel> AddPatient(UserModel patient) async {
    var response =
    await http.post(Uri.parse(API_URL), body: jsonEncode(patient));
    var jsonData = jsonDecode(response.body);
    UserModel patient;
    if (response.statusCode == 201) {
      patient = UserModel.fromJson(jsonData);
    } else {
      throw Exception("Patient no saved");
    }
  ...
  // edit Patient
  Future<UserModel> EditPatient(UserModel patient) async {
    // put reqiest to update Patient
    var response =
    await http.put(Uri.parse(API_URL), body: jsonEncode(patient))
    );
    var jsonData = jsonDecode(response.body);
    UserModel patient;
    if (response.statusCode == 200) {
  ...
  DeletePatientByCin(String cin) async {
    // put reqiest to update Patient
    var response =
    await http.delete(Uri.parse(API_URL + cin));
    var jsonData = jsonDecode(response.body);
    UserModel patient;
